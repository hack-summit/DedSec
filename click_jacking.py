...
class click_jacking():
    ...
    def click_jackingfun(self):
        ...
        bool_flag = True
        print1 = "this isn't get request, so it is not vulnerable to clickjacking."
        a = r.headers
        if (request.method == "GET"):
            print1 = "this domain doesn't allow html as content-type.so, it is not vulnerable to click-jacking."
            if ("text/html" in a["Content-Type"]):
                bool_flag = False
